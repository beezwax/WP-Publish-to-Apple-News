...
class Admin_Action_Index_Delete_Test extends WP_UnitTestCase {
	...
	public function testActionPerform() {
		...
		update_post_meta( $post_id, 'apple_export_api_id', $remote_id );

		$action = new Delete( $this->settings, $post_id );
		$action->set_api( $api->reveal() );
		$action->perform();

		$this->assertNotEquals( null, get_post_meta( $post_id, 'apple_export_api_deleted', true ) );
		$this->assertEquals( null, get_post_meta( $post_id, 'apple_export_api_id', true ) );
	}

	public function testActionPerformWhenNotPushed() {
		// Expect an exception
		$this->setExpectedException( '\Actions\Action_Exception', 'This post has not been pushed to Apple News, cannot delete.' );
