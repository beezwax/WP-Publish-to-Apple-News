...
class Admin_Apple_Meta_Boxes extends Apple_News {
	...
	public function publish_meta_box( $post ) {
		...
		if ( ! empty( $api_id ) ) {
			...
			$created_at = empty( $created_at ) ? __( 'None', 'apple-news' ) : date( 'F j, h:i a', strtotime( $created_at ) );
			$modified_at = get_post_meta( $post->ID, 'apple_news_api_modified_at', true );
			$modified_at = empty( $modified_at ) ? __( 'None', 'apple-news' ) : date( 'F j, h:i a', strtotime( $modified_at ) );
			?>
