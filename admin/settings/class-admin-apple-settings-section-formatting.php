...
class Admin_Apple_Settings_Section_Formatting extends Admin_Apple_Settings_Section {
	...
	public function render_meta_component_order() {
		?>
		<ul id="meta-component-order-sort" class="component-order">
			<?php
				...
				if ( ! empty( $component_order ) && is_array( $component_order ) ) {
					foreach ( $component_order as $component_name ) {
						echo sprintf(
							'<li id="%s" class="ui-state-default"><span class="ui-icon ui-icon-arrowthick-2-n-s"></span>%s</li>',
		...
		<p class="description"><?php esc_html_e( 'Set the order of the meta components at the top of the article. These include the title, the cover (i.e. featured image) and byline which also includes the date.', 'apple-news' ) ?></p>
		<?php
	}
